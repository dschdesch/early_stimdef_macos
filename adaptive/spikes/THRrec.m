function S = THRrec(dev, EXP, ToneFreq, BurstDur, NonBurstDur, MinSPL, MaxSPL, StartSPL, StepSPL, DAchan, SpikeDiffCrit, MaxNpres, GUI)
% THRrec - adaptive measurement of tonal threshold for one frequency
% Usage:
%   THRrec(dev, EXP, ToneFreq, BurstDur, NonBurstDur, MinSPL, MaxSPL, StartSPL, StepSPL, DAchan, SpikeDiffCrit, MaxNpres);
...
% TO DO: move this to makestimTHR.m
P.Fcar = ToneFreq;
P.BurstDur = 100;
P.ISI = 100;
P.Experiment = EXP;
...
while 1,
    ...
    if isReady
        ...
        if iAmpNew > NSPL || iAmpNew < 1
            GUImessage(GUI, 'SPL boundaries reached while recording.', 'error',{});
            %THRstop();
            break
        else
            iAmp = iAmpNew;
...
if Namp>Nmax,
    isReady = 1;
    GUImessage(GUI, 'Maximum number of presentations reached.', 'error',{});
    THRstop();
end
