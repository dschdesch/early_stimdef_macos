function P = ARevcorStim(P, varargin); 
...
[LowFreq, HighFreq, NoiseSeed, ModFreq, ModDepth, ModStartPhase, ModTheta, ...
    ...
    condtype,notchW,conddB,tonedB,condBurstDur,PrecursorFlag,addprecursor,polarity ] ...
    ...
    P.condtype,P.notchW,P.conddB,P.tonedB,P.condBurstDur,P.PrecursorFlag,P.addprecursor,P.polarity );
...
[DAchanStr, LowFreq, HighFreq, NoiseSeed, ...
    ...
    SPL,condtype,notchW,conddB,tonedB,condBurstDur,PrecursorFlag,addprecursor ] ...
...
for ichan=1:Nchan,
    ...
    for icond=1:Ncond,
        ...
        P.Waveform(icond,ichan) = local_Waveform(chanStr, P.Experiment, Fsam, ...
            ...
            SPL(idx), P.SPLtype,...
            condtype(idx),notchW(idx),conddB(idx),tonefreq,tonedB(idx),condBurstDur(idx),PrecursorFlag(idx),addprecursor(idx),polarity);
    end
end
...
function  W = local_Waveform(chanChar, EXP, Fsam, ...
    ...
    SPL, SPLtype,...
    condtype,notchW,conddB,tonefreq,tonedB,condBurstDur,PrecursorFlag,addprecursor,polarity);


tonefreq = tonefreq*1000;

nsamp_noise = NsamplesofChain([BurstDur], Fsam/1e3);
nsamp_cond = NsamplesofChain([condBurstDur], Fsam/1e3);
if addprecursor==1
nsamp_total = NsamplesofChain([BurstDur,condBurstDur], Fsam/1e3);
else
    nsamp_total = NsamplesofChain([BurstDur], Fsam/1e3);
end
...
if addprecursor==1
    if PrecursorFlag==0
        ...
        if condtype=='T'
            [DL, Dphi] = calibrate(EXP, Fsam, chanChar, tonefreq);
            Ampcond = dB2A(tonedB+DL)*sqrt(2); % calibrated linear amplitude
            conditioner = tonecomplex(Ampcond, tonefreq, phase(1:length(tonefreq)), Fsam, condBurstDur); % ungated waveform buffer; starting just after OnsetDelay
            conditioner = ExactGate(conditioner, Fsam, condBurstDur, 0, RiseDur, FallDur);
...
% plot(t,w)
% subplot(2,3,2)
% plot(linspace(0,length(n)*dt,length(n)),n)
% subplot(2,3,3)
% plot(linspace(0,length(conditioner)*dt,length(conditioner)),conditioner)
% 
% subplot(2,3,4)
% absfft = abs(fft(w));
