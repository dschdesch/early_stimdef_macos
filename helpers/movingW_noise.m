...
error(local_test_singlechan(P,{'NoiseSeed', 'ISI'}));
...
[LowFreq, HighFreq, NoiseSeed,ISI, OnsetDelay, BurstDur, RiseDur, FallDur, SPL,...
    notchW,BF,n_octaves,interval,order,ftype,speed,minspeed] ...
    = SameSize(P.LowFreq, P.HighFreq, P.NoiseSeed, ...
    P.ISI, P.OnsetDelay, P.BurstDur, P.RiseDur, P.FallDur, P.SPL,...
    P.notchW,P.BF,P.n_octaves,P.interval,P.order,P.ftype,P.speed,P.minspeed);
...
[DAchanStr, LowFreq, HighFreq, NoiseSeed, ...
    ...
    notchW,BF,n_octaves,interval,order,ftype,speed] ...
...
for ichan=1:Nchan,
    ...
    for icond=1:Ncond,
        ...
        P.Waveform(icond,ichan) = local_Waveform(chanStr, P.Experiment, Fsam, ...
            ...
            SPL(idx), P.SPLtype,notchW(idx),BF(idx),n_octaves(idx),interval(idx),order(idx),ftype(idx),speed(idx));
...
function  W = local_Waveform(chanChar, EXP, Fsam, ...
    ...
    SPL, SPLtype,notchW,BF,n_octaves,interval,order,ftype,speed);

speed=speed;
f1 = BF/(2^(n_octaves/2));
f2 = BF*(2^(n_octaves/2));
BurstDur = abs((f2-f1)/speed);
...
nsamples = length(n);
if speed>0
fcs = floor(2.^linspace(log2(f1),log2(f2),nsamples));
else
fcs = floor(2.^linspace(log2(f2),log2(f1),nsamples));
end

floor(interval*1e-3*Fsam);
fn = time_dep_filter(n',fcs,notchW,order,Fsam,filtertype,floor(interval*1e-3*Fsam))';
 
% gating
fn = ExactGate(fn, Fsam, BurstDur, 0, RiseDur, FallDur);
...
% plot(t,fn)
% subplot(223)
% spectrogram(n',lsegment,lsegment-100,NFFT,Fsam);
% xlim([0,8000])
% subplot(224)
